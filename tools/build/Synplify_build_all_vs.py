...
class Synthesize(ProjectDependency):
  def __init__(self, moduleList):
    ...
    for verilogTop in moduleList.getAllDependencies('VERILOG'):
      ...
      newPrjFile = open('config/' + verilogTop + '.modified.synplify.prj','w');  
    ...
    #once we get synth boundaries up, this will be needed only for top level
        ...
        newPrjFile.write('impl -add ' + verilogTop + ' -type fpga\n')
    ...
    #write the tail end of the options file to actually do the synthesis
        newPrjFile.write('set_option -frequency 100.00\n')
        newPrjFile.write('set_option -top_module ' + verilogTop + '\n')
        newPrjFile.write('project -result_file \"$env(BUILD_DIR)/' + moduleList.compileDirectory + '/' + verilogTop + '.edf\"\n')
    
        newPrjFile.write('project -run constraint_check\n');
        newPrjFile.write('project -run synthesis\n');
        newPrjFile.write('impl -active \"' + verilogTop + '\"\n');
    ...
    top_netlist = moduleList.env.Command(
        moduleList.compileDirectory + '/' +  verilogTop + '.edf',
        moduleList.topModule.moduleDependency['VERILOG'] + ['config/' + verilogTop + '.synplify.prj'] ,
        [ SCons.Script.Delete(moduleList.compileDirectory + '/' + verilogTop  + '.srr'),
          SCons.Script.Delete(moduleList.compileDirectory + '/' + verilogTop  + '_xst.xrpt'),
          'synplify_pro -batch config/' + verilogTop + '.modified.synplify.prj' ])
    SCons.Script.Clean(top_netlist,moduleList.compileDirectory + '/' + verilogTop + '.srp')
    SCons.Script.Clean(top_netlist,'config/' + verilogTop + '.modified.synplify.prj')
