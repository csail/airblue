...
// main
// The basic idea here is that we'll periodically send out a wireshark detectable packet.  We will also listen/report incoming packets. 
...
AIRBLUE_DRIVER_CLASS::Main()
{
   ...
   // Set up a packet for transmission.  This is some bogus TCP packet which shows up nice and red in wireshark
   int u_second_delay = 1000;
   packetGenStub->SetDelay(MODEL_CLOCK_FREQ*u_second_delay); // Other Traffic will get in here, and we surely are not doing CSMA

   packetGenStub->SetEnable(~0);

   packetGenStub->SetRate(0);
   packetGenStub->SetLength(packetLengths[0]);
   for(int i = 0; i < packetLengths[0]; i++) {
     packetGenStub->SetPacketByte(i,packets[0][i]);
   }
