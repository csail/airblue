...
prevStateOut = oct2dec(prevStateOut);
trelOutputs = oct2dec(TREL.outputs);
...
%     for dest_st = 0 : (TREL.numStates -1)
%         sum1 = -1e9;
%         a = Alpha(time, prevStates(dest_st+1, :) + 1) + BranMetric(oct2dec(prevStateOut(dest_st+1, :)) +1, time)';
%         Alpha(time+1, dest_st+1) = max(a);
%         
...
incr_prob0 = Alpha(1:end-1,:) + Beta(2:end,TREL.nextStates(:,1)+1) + BranMetric(oct2dec(TREL.outputs(:,1))+1, :)';
incr_prob1 = Alpha(1:end-1,:) + Beta(2:end,TREL.nextStates(:,2)+1) + BranMetric(oct2dec(TREL.outputs(:,2))+1, :)';

total_prob_bit0 = ones(size(incr_prob0,1),1) * -1e9;
total_prob_bit1 = ones(size(incr_prob1,1),1) * -1e9;

for state = 1 : TREL.numStates
    total_prob_bit0 = ln_of_sum_of_exps( total_prob_bit0, incr_prob0(:,state) );
    total_prob_bit1 = ln_of_sum_of_exps( total_prob_bit1, incr_prob1(:,state) );
end

LLR = (total_prob_bit0 - total_prob_bit1)';
...
%     total_prob_bit0 = -1e9;
%     
% %     tpb0 = Alpha(time,:) + Beta(time+1, TREL.nextStates(:,1)+1) + BranMetric(oct2dec(TREL.outputs(:,1))+1, time)'
% %     tpb0 = max(tpb0);
%     
%     for orig_st = 0 : (TREL.numStates -1)
