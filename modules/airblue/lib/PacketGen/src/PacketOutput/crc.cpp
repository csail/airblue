 ...
 * CRC-32 routine
 *
 * $Id: crc32.c 30405 2009-10-08 15:10:43Z krj $
 *
 ...
 *      x^5 + x^4 + x^2 + x + 1
 */
const UINT32 crc32_ccitt_table[256] = {
...
UINT32
crc32c_calculate(const void *buf, int len, UINT32 crc)
{
...
UINT32
crc32_ccitt(const UINT8 *buf, UINT len)
{
	return (crc32_ccitt_seed(buf, len, CRC32_CCITT_SEED));
}

UINT32
crc32_ccitt_seed(const UINT8 *buf, UINT len, UINT32 seed)
{
	UINT i;
	UINT32 crc32 = seed;
...
UINT32
crc32(const UINT8* buf, UINT len)
{
	UINT32 c_crc;
        c_crc =  crc32_ccitt_seed(buf, len, CRC32_CCITT_SEED) ;
...
/*UINT32
crc32_802_tvb(tvbuff_t *tvb, UINT len)
{
